...
with st.sidebar:
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.button("Import Uploaded Project", use_container_width=True):
                    ...
                    if idx < n_nodes:
                        with row_cols[j]:
                            with st.container(border=True):
                                ...
                                existing_node = st.session_state.get("nodes", [])[idx] if idx < len(st.session_state.get("nodes", [])) else None
                                default_x = existing_node[0] if existing_node else float(idx)
                                default_y = existing_node[1] if existing_node else 0.0
                                
                                x = c1.number_input(
                                    "X coord",
                                    value=st.session_state[f"node_x_{idx}"] if f"node_x_{idx}" in st.session_state else default_x,
                                    format="%.4f",
                                    key=f"node_x_{idx}"
                                )
                                y = c2.number_input(
                                    "Y coord",
                                    value=st.session_state[f"node_y_{idx}"] if f"node_y_{idx}" in st.session_state else default_y,
                                    format="%.4f",
                                    key=f"node_y_{idx}"
                                )
                                nodes.append((x, y))
    ...
    with st.expander("Properties (Material + Section)", expanded=True):
        ...
        for i in range(n_properties):
            with st.container(border=True):
                ...
                # Split inputs and preview side-by-side
                col_inputs, col_preview = st.columns([5, 3])
                ...
                with col_inputs:
                    ...
                    with sub_col2:
                        ...
                        if section_type == "rectangular_bar":
                            ...
                            if thickness >= min(base1, base2, height) / 2:
                                st.error("⚠️ Wall thickness must be less than half the minimum dimension!")
                            ...
                            if thickness >= outer_side / 2:
                                st.error("⚠️ Wall thickness must be less than half the side length!")
                            ...
                            if tw >= b or tf >= h / 2:
                                st.error("⚠️ Invalid dimensions (thickness exceeds outer boundary)!")
    ...
    with col_right:
        ...
        with st.expander("Constraints (Boundary Conditions)", expanded=True):
            ...
            if n_constraints > 0:
                ...
                col1.markdown("**Node**")
                col2.markdown("**DOF / Direction**")
                col3.markdown("**Prescribed Value**")
        ...
        with st.expander("Point Loads", expanded=True):
            ...
            if n_loads > 0:
                ...
                col1, col2, col3 = st.columns([2, 3, 3])
                col1.markdown("**Node**")
                col2.markdown("**Direction**")
                col3.markdown("**Magnitude**")
            ...
            if n_elems_current == 0:
                ...
                if n_dist_loads > 0:
                    for i in range(n_dist_loads):
                        with st.container(border=True):
                            # Lay out each distributed load cleanly in a single horizontal row of columns
                            col1, col2, col3, col4, col5 = st.columns([1.5, 2, 2.5, 3, 3])
...
with tab2:
    st.header("Run Analysis & Verify Model")
    ...
    with col_left:
        ...
        with st.expander("Model Summary", expanded=True):
            ...
            col_m1.metric("Nodes", len(nodes))
            col_m2.metric("Elements", len(st.session_state.get("elements", [])))
            col_m3.metric("Properties", len(st.session_state.get("properties", [])))
            
            col_m4, col_m5, col_m6, col_m7 = st.columns(4)
            col_m4.metric("Constraints", len(st.session_state.get("constraints", [])))
            col_m5.metric("Springs", len(st.session_state.get("springs", [])))
            col_m6.metric("Point Loads", len(st.session_state.get("point_loads", [])))
            col_m7.metric("Dist. Loads", len(st.session_state.get("distributed_loads", [])))
                    ...
                    try:
                        ...
                        for element_id, magnitude_start, magnitude_end, direction, func_str, _ in st.session_state["distributed_loads"]:
                            ...
                            if mesh_el_ids is None:
                                raise ValueError(
                                    f"Distributed load references element {element_id}, which was not found in the mesh."
                                )
    ...
    with col_right:
        ...
        if len(nodes) >= 2 and len(st.session_state.get("elements", [])) >= 1:
            try:
                ...
                st.info("Visual preview will update once coordinates are valid.")
        else:
            st.info("💡 Define nodes and elements to preview the structure.")

        # Persistent Display of Results
        if "displacements" in st.session_state and "reactions" in st.session_state and "mesh" in st.session_state:
            st.divider()
            ...
            dpn = st.session_state["dpn"]
            
            res_tab1, res_tab2 = st.tabs(["Nodal Displacements", "Reaction Forces"])
            ...
            with res_tab2:
                if reactions:
                    ...
                    st.info("No reaction forces (no constraints defined).")
...
with tab3:
    ...
    if "structure_results" in st.session_state:
        ...
        tab_forces, tab_deformed, tab_stresses = st.tabs([
            "Force Diagrams",
            "Deformed Shape",
            ...
            with col_controls:
                ...
                if not use_auto_scale:
                    user_scale = st.number_input(
                        ...
                        key="deformed_user_scale_factor",
                    )
                ...
                show_original = st.checkbox(
                    ...
                    key="deformed_show_original_shape",
                )
                ...
                hide_subdiv_def = st.checkbox(
                    ...
                    key="deformed_hide_subdiv_nodes",
                )
            ...
            if orig_element_ids:
                ...
                with col_controls:
                    ...
                    use_section_query = st.checkbox(
                        ...
                        key="stresses_section_query_enabled",
                    )
                    ...
                    shear_theory = "Jourawski (classical)"
                    if is_reddy:
                        st.markdown("---")
                        shear_theory = st.radio(
                            ...
                            horizontal=False,
                            key="stresses_shear_theory_selector"
                        )
                
                with col_plots:
                    if selected_element_result is not None:
                        # Snapping message
                        if use_global_pos and hit is not None:
                            if not hit["is_on_structure"]:
                                st.warning(
                        ...
                        st.warning("No elements found in the mesh.")
            else:
